...
def generate_xml(products_json, categories_json):
    ...
    for product_name, product_info in products.items():
        ...
        if category:
            xml_output += f"<product name='{product_name}' categories='{category}'>\n"
        else:
            xml_output += f"<product name='{product_name}'>\n"
        xml_output += f"    <price>{product_info['price']:.2f}</price>\n"
        xml_output += f"    <quantity>{product_info['quantity']}</quantity>\n"
        xml_output += '</product>\n'
...
print(generate_xml(products_json, categories_json))
